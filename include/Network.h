...
namespace SNEEZE
{
   ...
   public:
      ...
      {
      public:
         FILE (INETWORK_IMPL* pINetwork_Impl, CONTAINER::CID* pCID, uint32_t nFileIx, const std::string& sUrl, const std::string& sHash, bool bCacheEnabled);
         ~FILE ();
...
//         std::string          Header (const std::string& sName) const;
      ...
      FILE* File_Open (CONTAINER::CID* pCID, const std::string& sUrl, IFILE* pListener);
      FILE* File_Open (CONTAINER::CID* pCID, const std::string& sUrl, const std::string& sHash, uint32_t nAssetIx = 0, IFILE* pListener = nullptr);
